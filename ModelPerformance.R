...
name <- "UnigramFeatures"
database <- 4
perc <- 0.75
cols.to.del <- c("username", "location", "userop", "quote", "rating", "date",
                 "reviewnospace", "page", "titleopinion")


#------------------------------------------------------------------------------#
# Load the dataframe and split into TRAIN and TEST
df <- splitTrainTest(name, database, perc, cols.to.del);

df.TEST <- df$test
df.ID.TEST <- df$idTest
df.TRAIN <- df$train
df.label.TEST <- df$labelTest
...
# Model training
time1 <- Sys.time()
ModelResults<- caret::train(SentimentValue ~ ., data=df.TRAIN,method="J48",
...
pathResults <- "D:/TFG-/Resultados/daliResults"

sink(pathResults)
print(paste0("Model: c4.5"))
print(paste0("DataSet: Unigram"))

print(paste0("Number of train instances: ", nrow(df.TRAIN)))
print("Model summary: ")
print(ModelResults)
...
confTable <- (nrow(df.TRAIN)/100)*(confMatrix$table)
# Precision: tp/(tp+fp):
...
# Prediction
for(i in 1:ncol(df.ID.TEST)){
  df.TEST[,i] <- as.numeric(as.character(df.TEST[,i]))
}

ModelResults_pred <- predict(ModelResults, df.ID.TEST, type="prob")
# print(confusionMatrix(xfbResults_pred[,2], LABELTripAdvisorFeaturesTEST))

df.Prediction <- data.frame(df.ID.TEST,df.label.TEST,ModelResults_pred[,2])
beep(2)
# setnames(df.Prediction, old=c("IDTripAdvisorFeaturesTEST", "LABELTripAdvisorFeaturesTEST", "ModelResults_pred...2."),
